...
2. **Tag language features**: add hover, references, definition, and rename support for inline and frontmatter tags.
3. **Document and workspace symbols**: expose headings, frontmatter keys, aliases, tags, links, note IDs, titles, and aliases through `textDocument/documentSymbol` and `workspace/symbol`.
4. **More code actions**: add diagnostic-driven fixes for duplicate IDs and aliases, link format conversion, and fixes for broken heading anchors.
5. **Performance and freshness**: add file watching or workspace file-operation handling, plus a cache/index layer so diagnostics, completion, hover, and navigation stay fresh without reparsing the whole vault for every request.
